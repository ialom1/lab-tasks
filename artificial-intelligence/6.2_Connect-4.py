...
class Info(Frame):
    def __init__(self, master=None):
        ...
        self.t = Label(self, text="Yellow Tower", font=police)
...
class Terrain(Canvas):
    ...
    def detCol(self, event):
        if self.perm:
            ...
            if self.joueur == 1:
                self.joueur = 2
                info.t.config(text="Red Tower")
                self.coul = "red"

            elif self.joueur == 2:
                self.joueur = 1
                info.t.config(text="Yellow tower")
                self.coul = "yellow"
